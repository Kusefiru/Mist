<script>
    ...
    let {
        ...
        getHref,
        title,
        subtitle: renderSubtitle // destructure
    } = $props();
    ...
    let canScrollRight = $derived(scrollOffset + visibleCount < items.length);
    ...
    function scrollRight() {
        if (canScrollRight) {
            scrollOffset = Math.min(items.length - visibleCount, scrollOffset + 2);
        }
    }
...
<div class="relative flex flex-col w-full gap-4">
    ...
    <div class="flex items-center justify-between gap-2">
        <h2 class="text-2xl font-bold text-ink-900 select-none">
            {@render title()}
        </h2>
